...
export default function PhotoUploader({ photos, onChange }: any) {
    ...
    function uploadPhoto(ev: any) {
        ...
        axios.post('/upload', data, {
            ...
            onChange([...photos, ...fileNames as never])
            setPhotoLink('')
        })
    }
    ...
    async function addPhotoByLink(ev: any) {
        ev.preventDefault();
        const { data: fileName }: never = await axios.post('/upload-by-link', { link: photoLink })
    ...
    function removePhoto(ev: any, filename: string) {
        ev.preventDefault();
        onChange([...photos.filter((p: string) => p !== filename)]);
    }

    function selectAsMain(ev: any, filename: string) {
        ev.preventDefault();
        const filteredPhotos : string[] = [...photos.filter((p : string) => p !== filename)]
        const newPhotos : string[] = [filename, ...filteredPhotos];
        onChange(newPhotos);
    }
    return (
        <>
            ...
            <div className="mt-2 grid gap-2 grid-cols-3 md:grid-col-4 lg:grid-col-6">
                {
                    photos?.length > 0 && photos?.map((link : string) => (
                        <div className="max-h-64 rounded-md bg-gray-100 place-content-center relative" key={link}>
                            ...
                            {
                             link === photos[0] ? 
                             <button onClick={(ev) => selectAsMain(ev, link)} className="cursor-pointer absolute bottom-1 left-1 text-white bg-black py-2 px-3 bg-opacity-50 rounded-2xl">
                ...
                <label className="flex items-center justify-center gap-1 border bg-transparent rounded-md p-8 text-2xl text-gray-600 cursor-pointer">
